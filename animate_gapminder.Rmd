...
1. Colors are tricky. Try looking for help about the gapminder package. Does it have some built-in colors that might be useful? DONE
1. Too much opaque overlapping for the datapoints is ugly. Our target has the points as somewhat transparent. How do we achieve that?  DONE
1. Our target graphic has no legend, which I think is right. How do we turn off the legend? DONE
1. The target uses data from 3 (?) continents. Filter our data for that case. DONE
1. How can we create three separate panes? DONE
